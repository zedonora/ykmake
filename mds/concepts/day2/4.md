...
`remix-themes`를 사용한 테마 토글은 `next-themes`와 다른 방식으로 동작합니다.

*   **`useTheme` 훅:**
    *   `remix-themes`에서 제공하는 `useTheme` 훅은 현재 적용된 테마 상태(`theme`)와 테마를 설정하는 함수(`setTheme`) 대신, **현재 테마 값만** 반환합니다. (`const [theme] = useTheme();`)
    *   이 훅은 서버에서 설정된 쿠키 값을 읽어 현재 테마를 클라이언트 컴포넌트에 제공합니다.
*   **테마 변경 메커니즘:**
    *   사용자가 드롭다운 메뉴에서 '라이트' 또는 '다크'를 선택하면, 단순히 클라이언트 상태를 변경하는 것이 아니라 **서버 액션(Action)을 호출**해야 합니다.
    *   `@remix-run/react`의 `useFetcher` 훅을 사용하여 `/action/set-theme` 엔드포인트로 `POST` 요청을 보냅니다. 요청 본문에는 사용자가 선택한 테마 값 (`theme=dark` 또는 `theme=light`)을 포함합니다.
    *   `useFetcher`는 페이지 전환 없이 백그라운드에서 서버와 통신할 수 있게 해줍니다.
    *   테마 변경 예시:
        ...
        const handleThemeChange = (theme: Theme) => {
          fetcher.submit(
            { theme: theme }, // 요청 본문 (form data)
            { method: "post", action: "/action/set-theme" } // 요청 정보
          );
        };
...
*   **서버 액션 (`/action/set-theme`):**
    *   이 라우트(`mds/jobs/day2/3.md`에서 정의)는 `POST` 요청을 받아 요청 본문의 `theme` 값을 읽습니다.
    *   `themeSessionResolver`를 사용하여 사용자가 선택한 테마 값을 세션 쿠키에 저장합니다.
    *   쿠키가 업데이트되면, Remix는 페이지를 다시 로드하거나 필요한 부분을 업데이트하여 변경된 테마를 반영합니다. (`root.tsx`에서 `useTheme` 훅이 새 쿠키 값을 읽어 `<html>` 태그의 클래스를 변경)

*   **`next-themes`와의 차이점:**
    *   `next-themes`는 주로 클라이언트 사이드에서 테마를 관리하고 `localStorage`를 사용합니다. `setTheme` 함수가 클라이언트 상태와 `localStorage`를 직접 업데이트합니다.
    *   `remix-themes`는 서버 측 쿠키를 사용하여 테마를 관리하며, 테마 변경은 서버 액션을 통해 이루어집니다. 이는 서버 렌더링 시에도 올바른 테마를 적용하는 데 유리합니다.
...
*   `remix-themes`는 서버 쿠키 기반으로 테마를 관리하며, 테마 변경은 `useFetcher`를 사용한 서버 액션 호출을 통해 이루어집니다.
