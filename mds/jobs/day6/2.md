    ...
    *   `user_id`는 `profiles` 테이블을 참조하도록 설정할 수 있습니다. (예: `ON DELETE SET NULL`)
    ...
    export const ideasGpt = pgTable('ideas_gpt', {
        ...
        return {
            ...
            userIdx: index("ideas_user_idx").on(table.userId),
        };
    });
...
3.  **`community.new.tsx` UI 및 Zod 스키마 구현:**
    ...
    *   `action` 함수에서 사용할 **Zod 스키마 (`PostSchema`)** 를 정의하여 유효성 검사 규칙을 명시합니다.
    ...
    import { type MetaFunction, type ActionFunctionArgs } from "@remix-run/node"; // Removed redirect for now
    ...
    declare const action: ActionFunctionArgs; // 임시 타입 선언 (useActionData 에러 방지)
        ...
        <div className="container mx-auto max-w-2xl px-4 py-8">
          ...
          <Form method="post" className="space-y-4">
            <div>
              ...
              {actionData?.errors?.title?.[0] && <p className="text-sm font-medium text-destructive">{actionData.errors.title[0]}</p>}
            </div>
            <div>
              <Label htmlFor="content">내용</Label>
              <Textarea id="content" name="content" rows={10} />
               {actionData?.errors?.content?.[0] && <p className="text-sm font-medium text-destructive">{actionData.errors.content[0]}</p>}
            </div>
            {actionData?.error && !actionData?.errors && ( // 필드 에러 아닌 일반 에러
