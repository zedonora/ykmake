...
YkMake는 개발자들을 위한 올인원 프로덕트 플랫폼으로, 아이디어부터 제품 출시까지 개발자의 전체 여정을 지원합니다.

## 핵심 기능

- **제품 쇼케이스**: 개발자들이 만든 제품을 전시하고 피드백을 받을 수 있는 공간
- **AI 아이디어 생성**: OpenAI 기반으로 새로운 프로젝트 아이디어를 얻을 수 있는 기능
- **개발자 매칭**: 프로젝트에 필요한 팀원을 찾거나 흥미로운 프로젝트에 참여할 수 있는 기능
- **커뮤니티 기능**: 개발자들 간의 지식 공유 및 네트워킹 지원
- **프로젝트 관리**: 자신의 프로젝트를 생성, 관리, 공유할 수 있는 기능

## 기술 스택

- **프론트엔드**: Remix, TailwindCSS, Shadcn UI
- **백엔드**: Supabase, DrizzleORM
- **인증**: 이메일/비밀번호, 소셜 로그인(Github, Google, Kakao)
- **배포**: Docker, GitHub Actions
- **AI 통합**: OpenAI

## 개발 진행 현황

현재 YkMake 프로젝트는 약 16일 차 개발 단계로, 다음과 같은 주요 작업들이 완료되었습니다:

1. **기본 구조 설정**: Remix 프로젝트 생성, 폴더 구조 설정, TailwindCSS 및 Shadcn UI 통합
2. **핵심 페이지 구현**: 제품 리스팅, 커뮤니티, 프로젝트 관리, 인증, 프로필 페이지 등
3. **인증 시스템**: 로그인, 회원가입, 소셜 로그인, OTP 인증 구현
4. **배포 준비**: 환경 변수 설정, 보안 설정, 성능 최적화, Docker 설정 완료

## 향후 계획

남은 개발 일정에는 다음과 같은 작업들이 계획되어 있습니다:

- 데이터베이스 테이블 구현 및 데이터 연동
- AI 아이디어 생성 시스템 구현
- 실시간 메시징 시스템 구현
- 결제 프로세스 구현
- 최종 테스트 및 문서화

YkMake는 30일 내에 모든 기능을 구현하는 것을 목표로 하고 있으며, 현재 배포 준비 단계까지 성공적으로 진행되고 있습니다.
